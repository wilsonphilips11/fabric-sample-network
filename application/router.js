...
sampleNetworkRouter.route('/create-asset').post(function (request, response) {
    submitTx(request, 'CreateAsset', request.body.id, request.body.color, request.body.size, 
        request.body.owner, request.body.appraisedValue).then((result) => {
        response.status(STATUS_SUCCESS);
        response.send(JSON.parse(result));
    }, (error) => {
        response.status(STATUS_SERVER_ERROR);
...
sampleNetworkRouter.route('/update-asset').put(function (request, response) {
    submitTx(request, 'UpdateAsset', request.body.id, request.body.color, request.body.size, 
        request.body.owner, request.body.appraisedValue).then((result) => {
        response.status(STATUS_SUCCESS);
